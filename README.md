...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1Tm8QeZE4a?p=24)


# 绪论
## 1.1 选题背景
当人们发现随着生产规模的不断扩大，人为计算方面才是一个巨大的短板，所以发明了各种计算设备，从结绳记事，到算筹，以及算盘，到如今的计算机，都是在无法满足生产的前提下出现的。随着计算机的发展，又出现了互联网技术。到现在为止，互联网已经发展了几十年了，在几十年的时间里就已经风靡世界。各行各业都发现了计算机的好处，计算机刚开始是军用的，后来在民用行业开始使用，到互联网时代，各种行业信息如井喷一般充斥着互联网，信息产生和传播的速度不断的提高。针对互联网的优点，结合互联网，对传统行业信息处理技术进行升级是非常有必要的。本课题对于物流信息的管理方面，开发一个物流信息管理系统，在信息管理方面不至于混乱，也能降低数据的出错率，数据安全方面也有了保证，该系统还有其他的优点，比如优化信息处理流程，降低信息泄露风险，减少资金投入，产出更高，让管理人员的工作更有效率等。所以说，物流信息管理系统是目前不可缺的，对使用者相当的重要。
## 1.2 选题意义
如今的年代，已经是步入信息社会了，不仅信息更新速度频繁，信息量也大，在信息时代必须有相应的处理信息的方法，如果还采用以前的结绳记事或者笔写纸记，不仅是信息录入效率上赶不上节奏，在信息检索的速度上更是让人无法承受。幸而当今社会上计算机技术发展的相当不错，可以通过计算机在信息处理上面实现自动化或者半自动化的作业，采用计算机技术，能有效的提高信息录入以及信息检索的效率，社会上相同行业之间本身就是效率高的淘汰效率低的，既然采用计算机来替代手工记录，必然是效率更高，稳定性更强，成本更低等诸多优点。针对于物流信息管理，开发一个物流信息管理系统不仅可以实现现代化的信息管理，也更符合现代化信息管理规范。

在实际的使用效果中，物流信息管理系统的意义如下：

第一点：物流信息管理系统的出现，就是为了提高工作人员的效率，能够在规定时间完成工作任务。
...
# 3 系统分析
对于物流信息管理系统开发设计到的流程有，分析系统的功能，设计系统的结构，设计数据库，编码以及测试，其中，在系统分析中，所做的工作包括功能的确定，性能的分析等。
## 3.1 可行性研究
物流信息管理系统开发实现分析需要从不同的角度来进行分析可行性，比如从时间角度，经济角度，甚至操作角度。从不同的角度分析可行性会让物流信息管理系统开发具体化，进而达到辩证开发的正确性。
### 3.1.1 经济可行性
从经济方面分析是第一要素，没有经济的支持，任何项目都如水中捞月，无法实现。实现物流信息管理系统，开发过程不需要额外的经济条件，用本人现有的计算机就可以实现，这方面不需要额外的支出。
### 3.1.2 时间可行性
物流信息管理系统设计主要作为毕业设计，在题目确定之后，答辩之前使用的项目，对不同的开发进度上面都有时间的要求，总不至于答辩完成后才能实现功能，这个肯定不行，所以从时间上来分析项目的工作量，发现是可行的，符合正常开发时间。
### 3.1.3 操作可行性
...
物流信息管理系统把操作该系统的用户群分为两类，即管理员，用户。

管理员对于物流信息管理系统操作的功能包括管理公告，管理收货地址发货地址信息，管理订单，管理快递信息，管理运货路线等功能。其用例图如图3.4所示：

![](/md/blog.004.png)

图3.4 管理员用例图

用户对于物流信息管理系统操作的功能包括查询快递，查看订单，查看运货路线，管理收货地址，管理发货地址，查看公告等。其用例图如图3.5所示：
...
## 4.2 功能模块设计
对管理员具体功能的设计结果将以图4.1所示的管理员功能结构图来进行体现。管理员对于物流信息管理系统操作的功能包括管理公告，管理收货地址发货地址信息，管理订单，管理快递信息，管理运货路线等功能。

![](/md/blog.006.png)

图4.1 管理员功能结构图

对用户具体功能的设计结果将以图4.2所示的用户功能结构图来进行体现。用户对于物流信息管理系统操作的功能包括查询快递，查看订单，查看运货路线，管理收货地址，管理发货地址，查看公告等。
...
（2）订单包括的属性有运输物品名称，发货地址，订单状态等。其属性图如下。

![](/md/blog.009.png)

图4.5 订单实体属性图

（3）运货路线包括的属性有费用，运货路线编号，大概时长等。其属性图如下。
...
| :-: | :-: | :-: | :-: |
|id (主键)|主键|int(11)|否|
|dingdan\_uuid\_number|订单号 |varchar(200)|是|
|shangpin\_name|运输物品名称 |varchar(200)|是|
|shangpin\_types|运输物品类型 |int(11)|是|
|yunshu\_content|运输物品详情|text|是|
|fahuoaddress\_id|发货地址|int(11)|是|
|shouhuoaddress\_id|收货地址|int(11)|是|
|yonghu\_id|用户|int(11)|是|
|yunhuoluxian\_id|运货路线|int(11)|是|
|dingdan\_types|订单状态 |int(11)|是|
|insert\_time|订单添加时间|timestamp|是|
|dingdan\_delete|逻辑删除|int(11)|是|
...
|fahuoaddress\_name|发货人 |varchar(200)|否|
|fahuoaddress\_phone|发货电话 |varchar(200)|否|
|fahuoaddress\_dizhi|发货地址|varchar(200)|否|
...
| :-: | :-: | :-: | :-: |
|id (主键)|主键|int(20)|否|
|dingdan\_id|订单|int(20)|否|
|kuaidi\_name|快递公司 |varchar(200)|否|
|kuaidi\_danhao|单号 |varchar(200)|否|
|kuaidi\_types|快递状态 |int(11)|否|
|kuaidi\_text|快递详情|text|是|
|insert\_time|添加时间|timestamp|否|
|create\_time|创建时间|timestamp|否|
表4.5 收货地址表
...
|id (主键)|主键|int(20)|否|
|yonghu\_id|创建用户|int(20)|否|
|shouhuoaddress\_name|收货人 |varchar(200)|否|
|shouhuoaddress\_phone|收货电话 |varchar(200)|否|
|shouhuoaddress\_dizhi|收货地址|varchar(200)|否|
|insert\_time|添加时间|timestamp|否|
|update\_time|修改时间|timestamp|否|
|create\_time|创建时间|timestamp|否|
表4.6 管理员表
...
| :-: | :-: | :-: | :-: |
|id (主键)|主键|int(20)|否|
|yunhuoluxian\_uuid\_number|运货路线编号 |varchar(200)|是|
|yunhuoluxian\_chufa|何处出发|varchar(200)|否|
|yunhuoluxian\_hechu|去往何处|varchar(200)|否|
|yunhuoluxian\_shichang|大概时长|varchar(200)|否|
|yunhuoluxian\_feiyong|费用(元)/公斤|decimal(10,2)|是|
|insert\_time|添加时间|timestamp|否|
|create\_time|创建时间|timestamp|否|
...
## 5.1 管理员功能实现
### 5.1.1 订单管理
该功能主要用于实现对订单基本信息的管理，订单管理界面的运行效果见图5.1。在此界面，管理员添加新的订单，对订单的运输物品名称，订单状态等信息进行修改，对订单信息进行删除，查询等。

![](/md/blog.013.png)

图5.1 订单管理界面
### 5.1.2 运货路线管理
该功能主要用于实现对运货路线基本信息的管理，运货路线管理界面的运行效果见图5.2。在此界面，管理员添加运货路线信息，运货路线信息包括费用，去往何处，何处出发，大概时长等信息。登记出错的运货路线信息也能进行更改。查询运货路线信息需要根据运货路线编号才能查询。

![](/md/blog.014.png)

图5.2 运货路线管理界面
### 5.1.3 快递单号查询
该功能主要用于实现对快递信息的查询和管理，快递单号查询界面的运行效果见图5.3。在此界面，管理员负责添加快递信息，快递信息包括快递公司，快递状态，快递详情等，登记错误的快递信息也能进行更改，查询快递可以使用快递单号查询，也能根据快递公司查询等。
...
图5.4 公告信息管理界面
### 5.1.5 用户管理
该功能主要用于实现对用户信息的管理。用户管理界面的运行效果见图5.5。在此界面，管理员更改用户信息，查询用户信息，删除用户等。
...
## 5.2 用户功能实现
### 5.2.1 发货地址管理
发货地址管理界面的运行效果见图5.6。在此界面，用户添加发货地址，对发货地址进行更改，发货地址主要包括发货人，发货电话，发货地址等信息，用户对发货地址可以进行批量删除等。

![](/md/blog.018.png)

图5.6 发货地址管理界面
### 5.2.2 订单查看
订单查看界面的运行效果见图5.7。在此界面，用户查询订单，对订单的运输物品名称，订单状态，运货路线编号，收货电话等信息进行查看。

![](/md/blog.019.png)

图5.7 订单查看界面
### 5.2.3 快递单号查询
快递单号查询界面的运行效果见图5.8。在此界面，用户查询快递，可以选择使用快递单号查询快递，可以选择使用快递公司查询快递，也能根据快递状态筛选快递信息等。
